-- Complete the revision trigger coverage for every posts column that can alter
-- the publisher snapshot. View counters remain intentionally excluded.
DROP TRIGGER IF EXISTS mahoon_public_content_revision_after_update;

CREATE TRIGGER mahoon_public_content_revision_after_update
AFTER UPDATE OF
  text,
  created_at,
  slug,
